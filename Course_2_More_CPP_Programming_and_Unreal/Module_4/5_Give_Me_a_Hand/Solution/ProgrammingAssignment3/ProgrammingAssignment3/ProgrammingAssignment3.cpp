...
int main()
{
	...
	while (Input[0] != 'q')
	{
		...
		for (int iter = 0; iter < NumCards; iter++) {
			// Take a card a card from the Deck
			Card card = GetCard();
			...
			Hand.push_back(card);
			// Count the suit
			std::string current_suit = card.GetSuit();
			if (current_suit == "Clubs") {
				amountClubs++;
			}
			else if (current_suit == "Diamonds") {
				amountDiamonds++;
			}
			else if (current_suit == "Hearts") {
				amountHearts++;
			}
			else if (current_suit == "Spades") {
				amountSpades++;
			}

		}
		std::cout << 'C' << amountClubs << ' '
			<< 'D' << amountDiamonds << ' '
			<< 'H' << amountHearts << ' '
			<< 'S' << amountSpades << '\n';
