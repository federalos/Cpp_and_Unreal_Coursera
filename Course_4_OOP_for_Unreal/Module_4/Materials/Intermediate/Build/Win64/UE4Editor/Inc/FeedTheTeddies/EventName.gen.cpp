// Copyright Epic Games, Inc. All Rights Reserved.
/*===========================================================================
	Generated code exported from UnrealHeaderTool.
	DO NOT modify this manually! Edit the corresponding .h files instead!
===========================================================================*/

#include "UObject/GeneratedCppIncludes.h"
#include "FeedTheTeddies/EventName.h"
#ifdef _MSC_VER
#pragma warning (push)
#pragma warning (disable : 4883)
#endif
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void EmptyLinkFunctionForGeneratedCodeEventName() {}
// Cross Module References
	FEEDTHETEDDIES_API UEnum* Z_Construct_UEnum_FeedTheTeddies_EventName();
	UPackage* Z_Construct_UPackage__Script_FeedTheTeddies();
// End Cross Module References
	static UEnum* EventName_StaticEnum()
	{
		static UEnum* Singleton = nullptr;
		if (!Singleton)
		{
			Singleton = GetStaticEnum(Z_Construct_UEnum_FeedTheTeddies_EventName, Z_Construct_UPackage__Script_FeedTheTeddies(), TEXT("EventName"));
		}
		return Singleton;
	}
	template<> FEEDTHETEDDIES_API UEnum* StaticEnum<EventName>()
	{
		return EventName_StaticEnum();
	}
	static FCompiledInDeferEnum Z_CompiledInDeferEnum_UEnum_EventName(EventName_StaticEnum, TEXT("/Script/FeedTheTeddies"), TEXT("EventName"), false, nullptr, nullptr);
	uint32 Get_Z_Construct_UEnum_FeedTheTeddies_EventName_Hash() { return 3516520090U; }
	UEnum* Z_Construct_UEnum_FeedTheTeddies_EventName()
	{
#if WITH_HOT_RELOAD
		UPackage* Outer = Z_Construct_UPackage__Script_FeedTheTeddies();
		static UEnum* ReturnEnum = FindExistingEnumIfHotReloadOrDynamic(Outer, TEXT("EventName"), 0, Get_Z_Construct_UEnum_FeedTheTeddies_EventName_Hash(), false);
#else
		static UEnum* ReturnEnum = nullptr;
#endif // WITH_HOT_RELOAD
		if (!ReturnEnum)
		{
			static const UE4CodeGen_Private::FEnumeratorParam Enumerators[] = {
				{ "GameOverEvent", (int64)GameOverEvent },
				{ "PointsAddedEvent", (int64)PointsAddedEvent },
			};
#if WITH_METADATA
			const UE4CodeGen_Private::FMetaDataPairParam Enum_MetaDataParams[] = {
				{ "GameOverEvent.Name", "GameOverEvent" },
				{ "ModuleRelativePath", "EventName.h" },
				{ "PointsAddedEvent.Name", "PointsAddedEvent" },
			};
#endif
			static const UE4CodeGen_Private::FEnumParams EnumParams = {
				(UObject*(*)())Z_Construct_UPackage__Script_FeedTheTeddies,
				nullptr,
				"EventName",
				"EventName",
				Enumerators,
				UE_ARRAY_COUNT(Enumerators),
				RF_Public|RF_Transient|RF_MarkAsNative,
				EEnumFlags::None,
				UE4CodeGen_Private::EDynamicType::NotDynamic,
				(uint8)UEnum::ECppForm::Regular,
				METADATA_PARAMS(Enum_MetaDataParams, UE_ARRAY_COUNT(Enum_MetaDataParams))
			};
			UE4CodeGen_Private::ConstructUEnum(ReturnEnum, EnumParams);
		}
		return ReturnEnum;
	}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
#ifdef _MSC_VER
#pragma warning (pop)
#endif
